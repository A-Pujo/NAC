...
<body style="font-family : Raleway" class="antialiased min-h-screen static lg:flex" x-data="{sidebar : false, sidebarExpand : true, imgShow : false, imgSrc: '', imgTitle:'' }">
    <!-- Show Image -->
    <div x-show="imgShow" x-transition class="fixed top-0 bg-neutral-400 bg-opacity-75 h-screen w-full flex justify-center items-center z-10">
        <div class="relative flex flex-col h-screen p-40 items-center">
            <h2 class="absolute text-center left-0 top-0 w-full -top-1 text-16 font-bold text-base-100" x-text="imgTitle"></h2>
            <img @click.outside="imgShow = false" :src="imgSrc" class="rounded-md overflow-hidden bg-base-100">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-32 w-32 text-primary-100 absolute -top-1 -right-1 cursor-pointer " viewBox="0 0 20 20" fill="currentColor"
                @click="imgShow = false"
            >
