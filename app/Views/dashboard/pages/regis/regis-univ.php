...
<?php
    $pages = [
        ...
        'daful' => 'Daful Univ',
        'verif-absen-1-1' => 'Verifikasi Absen Univ TM Semifinal',
        'verif-absen-1-2' => 'Verifikasi Absen Univ Opening Ceremony',
        'verif-absen-1-3' => 'Verifikasi Absen Univ Webinar #1 ',
        'verif-absen-1-4' => 'Verifikasi Absen Univ FGD Essay',
        'verif-absen-1-5' => 'Verifikasi Absen Univ Fun Games ',
        'verif-absen-1-6' => 'Verifikasi Absen Univ Webinar #2 ',
        'verif-absen-2-7' => 'Verifikasi Absen Univ Webinar #3 ',
        'verif-absen-2-8' => 'Verifikasi Absen Univ TM Final ',
        'verif-absen-2-9' => 'Verifikasi Absen Univ Final Round',
        'verif-absen-2-10' => 'Verifikasi Absen Univ Closing Ceremony ',
            ...
            <div x-show="dropdown" @click.outside="dropdown = false">
                <ul>
                    <?php $i = 0; foreach($pages as $page) : $i++?>
                        <li><a href="<?= base_url('dashboard/regis-univ?page='.$pages_key[$i - 1]) ?>"><?= $page ?></a></li>
                    <?php endforeach ?>
                </ul>
            </div>
        </div>
      ...
      <div class="col-span-12">
          <?php 
                if(($_GET['page'] ?? false) && str_contains($_GET['page'], 'absen')){
                    echo $this->include('dashboard/pages/regis/univ/verif-absen');
                } elseif(($_GET['page'] ?? false)) {
                    echo $this->include('dashboard/pages/regis/univ/'.$_GET['page']);
                }
            ?>
      </div>
    </div>
